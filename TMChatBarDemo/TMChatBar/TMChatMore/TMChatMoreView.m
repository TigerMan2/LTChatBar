...
@class TMChatMoreItemView;

typedef void(^ItemViewAction)(TMChatMoreItemView *itemView);

@interface TMChatMoreItemView : UIView

@property (nonatomic, copy) ItemViewAction itemAction;

@end

@implementation TMChatMoreItemView
{
...
- (void)updateViewWithItem:(TMChatMoreItem *)item {
...
@interface TMChatMoreView ()<UIScrollViewDelegate>

@end

@implementation TMChatMoreView
{
...
- (void)loadMoreItems:(NSArray <TMChatMoreItem *> *)items {
    ...
    for (int i = 0; i < items.count; i ++) {
        ...
        TMChatMoreItem *item = items[i];
        TMChatMoreItemView *view = [[TMChatMoreItemView alloc] initWithFrame:CGRectMake(x, y, kMoreItemIconSize, kMoreItemH)];
        [view updateViewWithItem:item];
        view.tag = i;
        view.itemAction = ^(TMChatMoreItemView *itemView) {
