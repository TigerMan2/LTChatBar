...
@implementation TMChatEmojiPageView
...
- (void)setThemeStyle:(TMChatEmojiThemeStyle)themeStyle {
    
    if (_themeStyle != themeStyle) {
        _themeStyle = themeStyle;
        switch (themeStyle) {
            case TMChatEmojiThemeStyleSystem:
                [self didInitSystemEmoji];
                break;
            case TMChatEmojiThemeStyleCustom:
                [self didInitCustomEmoji];
                break;
            case TMChatEmojiThemeStyleGif:
...
- (void)didInitSystemEmoji {
    ...
    for (int i = 0; i < lines; i ++) {
        for (int j = 0; j < cols; j ++) {
            TMChatEmojiButton *button = [TMChatEmojiButton buttonWithType:UIButtonTypeCustom];
...
- (void)loadSystemEmoji:(NSArray *)emojiData {
    ...
    for (int i = 0; i < emojiData.count; i ++) {
        TMChatEmojiModel *model = emojiData[i];
        TMChatEmojiButton *button = self.buttons[i];
        button.hidden = NO;
    ...
    TMChatEmojiButton *button = self.buttons[emojiData.count];
    button.hidden = NO;
...
- (void)emojiButtonClick:(TMChatEmojiButton *)sender {
