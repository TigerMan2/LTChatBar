...
@property (nonatomic, strong) TMRecordButton *recordButton;
...
@implementation TMChatInputView
...
- (void)didInitialize {
    ...
    self.voiceButton = [self createButton:TMChatButtonTypeVoice action:clickSel];
    self.faceButton = [self createButton:TMChatButtonTypeFace action:clickSel];
    self.moreButton = [self createButton:TMChatButtonTypeMore action:clickSel];
    self.switchBarButton = [self createButton:TMChatButtonTypeSwitchBar action:clickSel];
    self.switchBarButton.hidden = YES;
    
    self.recordButton = [[TMRecordButton alloc] init];
    
    self.textView = [[TMTextView alloc] init];
    ...
    self.recordButton.recordTouchDownAction = ^(TMRecordButton *recordButton) {
        NSLog(@"开始录音");
    ...
    self.recordButton.recordTouchUpInsideAction = ^(TMRecordButton *recordButton) {
        NSLog(@"完成录音");
    ...
    self.recordButton.recordTouchUpOutsideAction = ^(TMRecordButton *recordButton) {
        NSLog(@"取消录音");
        [recordButton setButtonStateNormal];
    ...
    //持续调用
    self.recordButton.recordTouchDragInsideAction = ^(TMRecordButton *recordButton) {
        
    };
    self.recordButton.recordTouchDragOutsideAction = ^(TMRecordButton *recordButton) {
        
    };
    //中间状态
    self.recordButton.recordTouchDragExitAction = ^(TMRecordButton *recordButton) {
        NSLog(@"将要取消录音");
    ...
    self.recordButton.recordTouchDragEnterAction = ^(TMRecordButton *recordButton) {
        NSLog(@"继续录音");
...
- (void)loadBarItems:(NSArray <TMChatInputItem *> *)items {
    for (TMChatInputItem *item in items) {
...
#pragma mark - 设置按钮的图片
- (void)setButton:(TMChatButtonType)buttonType normalImageStr:(NSString *)normalImageStr selectedImageStr:(NSString *)selectedImageStr highLightImageStr:(NSString *)highLightImageStr {
    UIButton *button;
    switch (buttonType) {
        case TMChatButtonTypeVoice:
            button = self.voiceButton;
            break;
        case TMChatButtonTypeFace:
            button = self.faceButton;
            break;
        case TMChatButtonTypeMore:
            button = self.moreButton;
            break;
        case TMChatButtonTypeSwitchBar:
...
- (CGFloat)getTextViewContentH:(TMTextView *)textView {
...
- (void)layoutAndAnimateTextView:(TMTextView *)textView {
...
#pragma mark - 创建按钮
- (UIButton *)createButton:(TMChatButtonType)buttonType action:(SEL)sel {
