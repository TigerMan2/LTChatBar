...
@interface LTChatBar () <LTChatInputViewDelegate,LTChatEmojiViewDelegate,LTChatMoreViewDelegate>
...
#pragma mark - LTChatEmojiViewDelegate
...
#pragma mark - LTChatMoreViewDelegate
...
#pragma mark - LTChatBarDataSource
- (void)setDataSource:(id<LTChatBarDataSource>)dataSource
{
    ...
    if ([self.dataSource respondsToSelector:@selector(chatKeyBoardInputItems)]) {
        NSArray<LTChatInputItem *> *barItems = [self.dataSource chatKeyBoardInputItems];
        [self.inputView loadBarItems:barItems];
    }
    
    if ([self.dataSource respondsToSelector:@selector(chatKeyBoardMoreItems)]) {
        NSArray<LTChatMoreItem *> *moreItems = [self.dataSource chatKeyBoardMoreItems];
        [self.moreView loadMoreItems:moreItems];
    }
    
    if ([self.dataSource respondsToSelector:@selector(chatKeyBoardEmojiSubjectItems)]) {
        NSArray<LTChatEmojiThemeModel *> *themeItems = [self.dataSource chatKeyBoardEmojiSubjectItems];
...
#pragma mark - 懒加载
- (LTChatInputView *)inputView {
    if (!_inputView) {
        _inputView = [[LTChatInputView alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, kChatToolBarHeight)];
...
- (LTChatEmojiView *)emojiView {
    if (!_emojiView) {
        _emojiView = [[LTChatEmojiView alloc] initWithFrame:CGRectMake(0, kChatKeyBoardHeight - kEmojiViewHeight, kScreenWidth, kEmojiViewHeight)];
...
- (LTChatMoreView *)moreView {
    if (!_moreView) {
        _moreView = [[LTChatMoreView alloc] initWithFrame:CGRectMake(0, kChatKeyBoardHeight-kMorePanelHeight, kScreenWidth, kMorePanelHeight)];
